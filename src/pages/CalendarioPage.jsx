...
// Junta viajes, eventos, itinerarios y lugares planeados en un mapa { 'YYYY-MM-DD': [ev, ev, ...] }
  ...
  return useMemo(() => {
    ...
    trips.items.forEach(t => {
      ...
      while (cur <= end && guard < 60) {
        add(ymdLocal(cur), { id: `trip-${t.id}-${guard}`, color: 'trip', label: t.name, sub: t.destination, kind: 'trip', refId: t.id })
        cur.setDate(cur.getDate() + 1); guard++
      }
      ;(t.itinerary || []).forEach((it, idx) => {
        if (!it.date) return
        add(it.date, { id: `it-${t.id}-${idx}`, color: 'actividad', label: it.title, sub: t.name, time: it.time, kind: 'itinerario', refId: t.id })
      })
    })
    ...
    events.items.forEach(ev => {
      ...
      add(ymdLocal(next), { id: `ev-${ev.id}`, color: EVENT_TYPE_MAP[ev.type]?.color || 'otro', label: ev.title, time: ev.time, kind: 'evento', refId: ev.id, recurring: ev.recurring })
    })

    places.items.forEach(p => {
      if (p.status === 'pendiente' && p.plannedDate) {
        add(p.plannedDate, { id: `pl-${p.id}`, color: 'lugar', label: p.name, sub: p.city, kind: 'lugar', refId: p.id })
      }
    })
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Viajes, actividades y fechas importantes, todo en un solo lugar.</p>
        </div>
      </div>
      ...
      <div className="cal-legend">
        {CAL_LEGEND.map(l => (
          <span key={l.color}><span className={`cal-dot ${l.color}`}></span>{l.label}</span>
        ))}
      </div>
      ...
      <div className="upcoming">
        ...
        {upcoming.length === 0 && <p className="page-sub">Nada en el horizonte todavía.</p>}
        {upcoming.map(ev => (
          <div key={ev.id} className="upcoming-row" onClick={() => openDay(ev.dateStr)}>
            <span className={`cal-dot ${ev.color}`}></span>
            <div className="upcoming-info">
              <div className="upcoming-label">{ev.label}</div>
              <div className="upcoming-when">{ev.d.toLocaleDateString('es-PE', { day: 'numeric', month: 'short' })}{ev.time ? ' · ' + ev.time : ''}{ev.sub ? ' · ' + ev.sub : ''}</div>
            </div>
            <div className="upcoming-days">{ev.days === 0 ? 'Hoy' : ev.days === 1 ? 'Mañana' : `${ev.days}d`}</div>
          </div>
        ))}
      </div>
      ...
      <Sheet open={!!dayModal} onClose={() => setDayModal(null)}
        ...
        {dayModal && dayModal.list.map(ev => (
          <div key={ev.id} className="day-event-row">
            ...
            <div>
              <div className="upcoming-label">{ev.label}</div>
              {(ev.sub || ev.time) && <div className="upcoming-when">{ev.time ? ev.time + ' · ' : ''}{ev.sub || ''}</div>}
            </div>
          </div>
        ))}
  ...
  return (
    <>
      <div className="cal-header">
        <button className="btn-text" onClick={onPrev}>‹ Anterior</button>
        <div className="cal-title">{MONTH_NAMES[month]} {year}</div>
        <button className="btn-text" onClick={onNext}>Siguiente ›</button>
      </div>
          ...
          return (
            <div key={i} className={`cal-cell${isToday ? ' cal-today' : ''}${dayEvents.length ? ' has-events' : ''}`}
              ...
              <div className="cal-dots">
                {dayEvents.slice(0, 3).map((ev, idx) => <span key={idx} className={`cal-dot ${ev.color}`}></span>)}
...
function WeekView({ cursor, evMap, todayStr, onPrev, onNext, onDay }) {
  ...
  return (
    <>
      <div className="cal-header">
        <button className="btn-text" onClick={onPrev}>‹ Semana</button>
        ...
        <button className="btn-text" onClick={onNext}>Semana ›</button>
      </div>
            ...
            <div key={i} className={`week-row${isToday ? ' cal-today' : ''}`} onClick={() => dayEvents.length && onDay(dateStr)}>
              ...
              <div className="week-events">
                ...
                {dayEvents.map((ev, idx) => (
                  <span key={idx} className="week-chip"><span className={`cal-dot ${ev.color}`}></span>{ev.label}</span>
                ))}
              </div>
            </div>
          )
      ...
      {rows.map(r => (
        <div key={r.dateStr} className="agenda-group" onClick={() => onDay(r.dateStr)}>
          ...
          <div className="agenda-events">
            {r.events.map((ev, idx) => (
              <div key={idx} className="agenda-event"><span className={`cal-dot ${ev.color}`}></span>{ev.label}{ev.time ? ` · ${ev.time}` : ''}</div>
            ))}
          </div>
        </div>
      ))}
